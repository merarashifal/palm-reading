...
## Current Status: Pending Sprint A5 (WordPress Integration)
Engine development is now frozen. All focus shifts to building the customer-facing commercial layer as a **separate repository** (`palm-reading-wordpress`).

**Strategic Decisions for Sprint A5:**
1. **Clean Boundaries**: The WordPress plugin is not the product; it is just the first client of the Engine. It will live in its own repository.
2. **Hyper-Focus**: Build exactly one shortcode (`[palm_reader]`).
3. **Customer Flow**: Landing ➔ Upload ➔ Processing (JS anticipation) ➔ Free Report ➔ Premium CTA.
4. **Out of Scope**: No admin settings, no user logins, no databases, no elementor blocks.
5. **Engine Integration**: The plugin will act as a thin wrapper that calls the Engine's `DemoPipeline` and renders the output.
