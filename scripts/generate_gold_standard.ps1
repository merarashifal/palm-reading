...
foreach ($key in $sections.Keys) {
    ...
    $rule_uid = "PALM_LIFE_LONG_" + $key.ToUpper()
    ...
    $file_path = Join-Path $dir "$key.json"
    # ConvertTo-Json doesn't do unicode unescaping by default in powershell 5, but we can bypass it or just let it escape to \uXXXX
    $jsonContent = $jsonObj | ConvertTo-Json -Depth 10 
    # Quick fix for unicode escaping in powershell
    $jsonContent = [System.Text.RegularExpressions.Regex]::Unescape($jsonContent)
    
    Set-Content -Path $file_path -Value $jsonContent -Encoding UTF8
}

Write-Host "Generated 16 rules"
